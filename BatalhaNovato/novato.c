...
int main() {
    ...
    // Exibe as coordenadas exatas de cada parte dos navios
    // Essa parte atende ao requisito de mostrar só as posições ocupadas

    printf("\nCoordenadas do navio horizontal:\n");
    for (j = 0; j < 3; j++) {
        printf("[%d][%d]\n", linhaH, colunaH + j);  // Mostra cada célula ocupada
    }

    printf("\nCoordenadas do navio vertical:\n");
    for (i = 0; i < 3; i++) {
        printf("[%d][%d]\n", linhaV + i, colunaV);  // Mostra cada célula ocupada
    }

    // Exibe o tabuleiro no console (visualização geral)
